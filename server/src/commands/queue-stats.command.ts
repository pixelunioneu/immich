...
export class QueueStatsCommand extends CommandRunner {
  ...
  async run(): Promise<void> {
    try {
      const all = await this.jobService.getAllJobsStatus();
      const entries = Object.entries(all);
      this.printJson(entries);
    } catch (error) {
      console.error(error);
      console.error('Failed to retrieve queue stats. Ensure the job service is properly configured and the queue is accessible.');
    }
  }

  private printJson(entries: Array<[string, JobStats]>) {
    const queues: Record<string, JobStats> = {};
