...
const OidcTokenResponseSchema = z
  .object({
    accessToken: z.string().optional().describe('OIDC access token for use with PixelUnion APIs'),
    reauth: z.boolean().optional().describe('Whether the user must re-authenticate via OAuth'),
  })
  .meta({ id: 'OidcTokenResponseDto' });
...
export class OidcTokenResponseDto extends createZodDto(OidcTokenResponseSchema) {}
