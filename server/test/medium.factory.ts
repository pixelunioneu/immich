...
const newRealRepository = <T extends BaseServiceDeps[number]>(key: T, db: Kysely<DB>): InstanceType<T> => {
  switch (key) {
    ...
    // PixelUnion: SearchRepository injects ConfigRepository + PuApiRepository for geodata externalization.
    case SearchRepository: {
      return new key(
        db,
        new ConfigRepository(),
        new PuApiRepository(new ConfigRepository(), LoggingRepository.create()),
