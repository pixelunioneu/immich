...
export type AdminConfigServerDto = {
    ...
    /** Public users */
    publicUsers: boolean;
    /** Server-wide upload quota in GB */
...
export type OidcTokenResponseDto = {
    /** OIDC access token for use with PixelUnion APIs */
    accessToken?: string;
    /** Whether the user must re-authenticate via OAuth */
    reauth?: boolean;
};
...
/**
 * Exchange OIDC access token
 */
export function getOidcToken(opts?: Oazapfts.RequestOpts) {
    return oazapfts.ok(oazapfts.fetchJson<{
        status: 200;
        data: OidcTokenResponseDto;
    }>("/auth/oidc-token", {
        ...opts,
        method: "POST"
    }));
